...
onready var wait_time_timer = $WaitTime
...
func _ready():
	...
	randomize()
	seat_num = randi()%7+0
	coffe_type_num = randi()%6+1
	coffe_size_num = randi()%3+1
	coffe_sugar_num = randi()%2+1
	...
	if coffe_sugar_num == 1:
		wants_sugar = "yes"
		coffe_sugar_label.text = "X"
	if coffe_sugar_num == 2:
		wants_sugar = ""
		coffe_sugar_label.text = ""
...
func _on_WaitTime_timeout():
	...
	seats[seat_num].seat_is_already_taken = false
	path = exit_point
	navigation_agent.set_target_location(path.global_position)
